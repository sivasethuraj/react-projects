...
const Home = () => {
    ...
    const fetchListOfProducts = async () => {
        ...
        if ( localStorage.getItem( 'shopping-cart-items' ) ) {
            const data = JSON.parse( localStorage.getItem( 'shopping-cart-items' ) );
            setProducts( data );
        } else {
            ...
            if ( data && data.length > 0 ) {
                setProducts( data );
                localStorage.setItem( 'shopping-cart-items', JSON.stringify( data ) );
            }
        }
        setLoading( false );
    };
